    ...
    function QTip(target, options, id, attr) {
        ...
        */
        ...
        function createButton() {
            ...
            var pinClass = self.get('show.persistent') ? 'ui-icon-pin-s' : 'ui-icon-pin-w';

            var pinButton = $('<a />', {
                'class': 'ui-state-default ui-tooltip-close pinButton ' + (options.style.widget ? '' : uitooltip + '-icon'),
                'title': close,
                'aria-label': close
            })
			.prepend(
				$('<span />', {
				    'class': 'ui-icon ' + pinClass,
				    'html': ''
				})
			);

            pinButton.appendTo(elements.titlebar)
			.attr('role', 'button')
			.click(function (event) {
			    if (!tooltip.hasClass(disabled)) {


			        var pinButton = $('.pinButton span', $(self.elements.button).parent());

			        var pinned = self.get('show.persistent');

			        if (pinned) {
			            self.set('show.persistent', false);
			            pinButton.addClass('ui-icon-pin-w').removeClass('ui-icon-pin-s');
			        } else {
			            self.set('show.persistent', true);
			            pinButton.addClass('ui-icon-pin-s').removeClass('ui-icon-pin-w');
			        }
